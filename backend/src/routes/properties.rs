...
pub async fn update_property(
    ...
    if payload.address.is_some() { updates.push("address = ?"); }
    if payload.city.is_some() { updates.push("city = ?"); }
    if payload.state.is_some() { updates.push("state = ?"); }
    if payload.zip_code.is_some() { updates.push("zip_code = ?"); }
    if payload.property_type.is_some() { updates.push("property_type = ?"); }
    if payload.bedrooms.is_some() { updates.push("bedrooms = ?"); }
    if payload.bathrooms.is_some() { updates.push("bathrooms = ?"); }
    if payload.square_feet.is_some() { updates.push("square_feet = ?"); }
    if payload.purchase_price.is_some() { updates.push("purchase_price = ?"); }
    if payload.current_value.is_some() { updates.push("current_value = ?"); }
    ...
    if let Some(v) = &payload.address { q = q.bind(v); }
    if let Some(v) = &payload.city { q = q.bind(v); }
    if let Some(v) = &payload.state { q = q.bind(v); }
    if let Some(v) = &payload.zip_code { q = q.bind(v); }
    if let Some(v) = &payload.property_type { q = q.bind(v); }
    if let Some(v) = payload.bedrooms { q = q.bind(v); }
    if let Some(v) = payload.bathrooms { q = q.bind(v); }
    if let Some(v) = payload.square_feet { q = q.bind(v); }
    if let Some(v) = payload.purchase_price { q = q.bind(v); }
    if let Some(v) = payload.current_value { q = q.bind(v); }
