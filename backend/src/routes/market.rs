...
use crate::models::{TrendData, TrendPoint, MarketAnalytics};
use crate::scraper;
...
pub async fn get_trends(
    ...
    // Group by location
    let mut trends_map: std::collections::HashMap<String, Vec<TrendPoint>> = std::collections::HashMap::new();
    ...
    for (location, date_str, median_price, inventory_count) in data {
        ...
        trends_map.entry(location.clone())
    ...
    let trends: Vec<TrendData> = trends_map.into_iter()
...
pub async fn get_analytics(
    ...
    let market_trends: Vec<TrendData> = trends_map.into_iter()
        .map(|(location, time_series)| TrendData {
            location,
            time_series,
        })
